---
title: Message Protocol
description: Wire protocol used between client and server over MessagePort
---

import { Aside } from '@astrojs/starlight/components';

This page documents the low-level message protocol used over the MessagePort. You do not need to know this to use the library, but it is useful for debugging or building custom integrations.

<Aside type="note">
All messages are sent via `port.postMessage()` and received via the `message` event. Because MessagePort uses the Structured Clone algorithm, messages are not serialized to JSON -- objects are transferred natively.
</Aside>

When a tRPC data transformer is configured, request inputs, response data, and error shapes are transformed before they are posted to the port and transformed back after they are received. Values that still cannot be cloned by the platform Structured Clone algorithm, such as JavaScript `Proxy` objects without a transformer that converts them, cannot be sent over this transport.

## Client to Server Messages

### Request

Sent when the client initiates a query, mutation, or subscription.

```typescript
interface TRPCPortRequest {
  kind: 'request';
  id: number;
  method: 'query' | 'mutation' | 'subscription';
  path: string;
  input: unknown;
  lastEventId?: string;
}
```

| Field | Type | Description |
|---|---|---|
| `kind` | `'request'` | Message discriminator |
| `id` | `number` | Unique request identifier (monotonically increasing) |
| `method` | `'query' \| 'mutation' \| 'subscription'` | The tRPC procedure type |
| `path` | `string` | Dot-separated procedure path (e.g., `'user.getById'`) |
| `input` | `unknown` | The procedure input after any configured input transformer, transferred via Structured Clone |
| `lastEventId` | `string \| undefined` | For subscription resumption (optional) |

### Subscription Stop

Sent when the client unsubscribes from an active subscription.

```typescript
interface TRPCPortSubscriptionStop {
  kind: 'subscription.stop';
  id: number;
}
```

| Field | Type | Description |
|---|---|---|
| `kind` | `'subscription.stop'` | Message discriminator |
| `id` | `number` | The ID of the subscription to stop |

## Server to Client Messages

### Result (Data)

Sent when a procedure returns data or a subscription yields a value.

```typescript
interface TRPCPortResultData {
  kind: 'result';
  id: number;
  type: 'data';
  data: unknown;
  eventId?: string;
}
```

| Field | Type | Description |
|---|---|---|
| `kind` | `'result'` | Message discriminator |
| `id` | `number` | The request ID this result belongs to |
| `type` | `'data'` | Result type discriminator |
| `data` | `unknown` | The procedure result after any configured output transformer, transferred via Structured Clone |
| `eventId` | `string \| undefined` | The tracked event ID for resumable subscription data |

For **queries and mutations**, a single `data` result is sent, and the request is complete.

For **subscriptions**, multiple `data` results may be sent over time.

When a subscription yields `tracked(id, data)`, the protocol sends `eventId: id` and `data: { id, data }`, matching tRPC v11's tracked subscription output shape.

### Result (Started)

Sent after a subscription procedure has started successfully.

```typescript
interface TRPCPortResultStarted {
  kind: 'result';
  id: number;
  type: 'started';
}
```

| Field | Type | Description |
|---|---|---|
| `kind` | `'result'` | Message discriminator |
| `id` | `number` | The subscription ID |
| `type` | `'started'` | Indicates the subscription has started |

### Result (Stopped)

Sent when a subscription ends naturally (the async iterable completes).

```typescript
interface TRPCPortResultStopped {
  kind: 'result';
  id: number;
  type: 'stopped';
}
```

| Field | Type | Description |
|---|---|---|
| `kind` | `'result'` | Message discriminator |
| `id` | `number` | The subscription ID |
| `type` | `'stopped'` | Indicates the subscription has ended |

### Error

Sent when a procedure throws an error.

```typescript
interface TRPCPortError {
  kind: 'error';
  id: number;
  error: unknown;
}
```

| Field | Type | Description |
|---|---|---|
| `kind` | `'error'` | Message discriminator |
| `id` | `number` | The request ID that caused the error |
| `error` | `unknown` | The tRPC error shape after any configured output transformer |

## Message Flow Examples

### Query

```
Client                          Server
  |                               |
  |  { kind: 'request',          |
  |    method: 'query',          |
  |    id: 1, path, input }      |
  |------------------------------>|
  |                               |
  |  { kind: 'result',           |
  |    id: 1, type: 'data',      |
  |    data }                     |
  |<------------------------------|
```

### Subscription

```
Client                          Server
  |                               |
  |  { kind: 'request',          |
  |    method: 'subscription',   |
  |    id: 1, path, input }      |
  |------------------------------>|
  |                               |
  |  { kind: 'result',           |
  |    id: 1, type: 'started' }  |
  |<------------------------------|
  |  { kind: 'result',           |
  |    id: 1, type: 'data', ..}  |
  |<------------------------------|
  |  { kind: 'result',           |
  |    id: 1, type: 'data', ..}  |
  |<------------------------------|
  |                               |
  |  { kind: 'subscription.stop',|
  |    id: 1 }                   |
  |------------------------------>|
  |                               |
  |  // Or, if the async iterable |
  |  // completes naturally:      |
  |  { kind: 'result',           |
  |    id: 1, type: 'stopped' }  |
  |<------------------------------|
```
