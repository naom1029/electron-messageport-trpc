...
<Aside type="caution">
Create one terminating `mainPortLink()` for each MessagePort. Sharing the same port across multiple terminating link instances can collide request IDs.
</Aside>
...
<Aside type="caution">
This is a low-level API. The supplied port should be dedicated to the electron-messageport-trpc protocol; do not multiplex app-defined `postMessage()` traffic on the same port.
</Aside>
