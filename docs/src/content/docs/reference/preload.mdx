---
title: Preload API
description: API reference for electron-messageport-trpc/preload
---

import { Aside } from '@astrojs/starlight/components';

```typescript
import {
  exposeElectronTRPC,
  exposePortReceiver,
} from 'electron-messageport-trpc/preload';
```

## `exposeElectronTRPC(channels?)`

Sets up the preload bridge for the typed channel registry.

```typescript
import { exposeElectronTRPC } from 'electron-messageport-trpc/preload';
import { electronTRPC } from '../main/trpc';

exposeElectronTRPC(electronTRPC);
```

This is the primary preload API for `0.5.x`. It can receive multiple channel
ports and forward the requested channel into the renderer main world.

---

## `exposePortReceiver()`

Sets up the preload script to receive a MessagePort from the main process and expose it to the renderer.

### What It Does

1. Listens on the internal IPC channel for a port sent by `createRendererPort()`.
2. Uses `contextBridge.exposeInMainWorld()` to expose a `window.electronTRPCPort` object with a `requestPort()` method.
3. When the renderer requests a port, forwards the transferred MessagePort into the renderer's main world via `window.postMessage()`.

### Parameters

```typescript
interface ExposePortReceiverOptions {
  channels?: readonly string[];
}
```

### Returns

`void`

### Example

```typescript
// preload.ts
import { exposePortReceiver } from 'electron-messageport-trpc/preload';

exposePortReceiver();
```

<Aside type="caution">
This function must be called in the preload script, not in the main process or renderer. It uses `ipcRenderer` and `contextBridge` from Electron, which are only available in the preload context.
</Aside>

### What Gets Exposed

After calling `exposePortReceiver()`, the renderer has access to:

```typescript
window.electronTRPCPort.requestPort(channel?: string): void
```

You do not need to use this directly. `createElectronTRPCClient()` and
`getPort()` wrap it for you.

### Security

The function uses `contextBridge.exposeInMainWorld()` only for the request trigger. The actual `MessagePort` is transferred with `window.postMessage()`, so the renderer still receives a real browser `MessagePort`.
